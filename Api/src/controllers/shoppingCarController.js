...
const editShoppingCarController = async (req, res) => {
  ...
  try {
    ...
    res.send('Shopping cart added successfully');
  } catch (error) {
    res.status(400).json(error.message);
  }
};
...
const deleteShoppingCarController = async (req, res) => {
  ...
  try {
    ...
    res.send('Shopping cart deleted successfully');
  } catch (error) {
    res.status(400).json(error.message);
  }
};

// NUEVO: limpiar todo el carrito por userId
const clearShoppingCarByUserId = async (userId) => {
  await ShoppingCar.destroy({ where: { userId } });
};
